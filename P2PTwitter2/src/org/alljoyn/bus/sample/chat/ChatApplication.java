...
public class ChatApplication extends Application implements Observable {
	...
	/*
	 * public synchronized Status getOutboundStatus() { if
	 * (mOutboundStatuses.isEmpty()) { return null; } else { return
	 * mOutboundStatuses.remove(0); } }
	 */
	/**
	...
	private void addInboundItem(String statusString) {
		String[] statusArray = statusString.split(";");
		Status status = new Status(new User(statusArray[0]), new User(
