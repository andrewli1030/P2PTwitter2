...
public class AllJoynService extends Service implements Observer {
	...
	public void onCreate() {
		...
		/*
		 * Intent intent = new Intent(this, TabWidget.class); PendingIntent
		 * pendingIntent = PendingIntent.getActivity(this, 0, intent, 0);
		 * Notification notification = new Notification(R.drawable.icon, null,
		 * System.currentTimeMillis()); notification.setLatestEventInfo(this,
		 * title, message, pendingIntent); notification.flags |=
		 * Notification.DEFAULT_SOUND | Notification.FLAG_ONGOING_EVENT |
	...
	/*private void doSendStatuses() {
		...
		while ((message = mChatApplication.getOutboundStatus()) != null) {
			 ...
			 * try { if (mJoinedToSelf) { if (mHostChatInterface != null) {
			 * mHostChatInterface.Chat(message); } } else {
			 * mChatInterface.Chat(message); } } catch (BusException ex) {
			 * mChatApplication.alljoynError(ChatApplication.Module.USE,
			 * "Bus exception while sending message: (" + ex + ")"); }
	...
	class ChatService implements ChatInterface, BusObject {
		...
		/*
		 * @Override public void Chat(diesel.ali.Status status) throws
		 * BusException { }
		 */
	}
	...
	public void Chat(String string) {
		...
		// * Always drop our own signals which may be echoed back from the
		// system.
	...
	public void Chat(diesel.ali.Status status) {
		...
		Log.i(TAG, "Chat(): signal " + status + " received from nickname "
				+ nickname);
